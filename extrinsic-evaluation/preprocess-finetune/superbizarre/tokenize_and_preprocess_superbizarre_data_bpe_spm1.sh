...
do
  ...
  for SPLIT in $SPLITS
  do
    cp "$TASK_DATA_FOLDER/$SPLIT.tsv" "$TASK_DATA_FOLDER/processed_spm1_bpe/$SPLIT.tsv";
  ...
  rm -rf "../../data-bins/superbizarre/bpe/alpha0/vocab_size16000/spm1/$TASK-bin"
  ...
  do
    ...
    fairseq-preprocess \
      ...
      --destdir "../../data-bins/superbizarre/bpe/alpha0/vocab_size16000/spm1/$TASK-bin/$LANG" \
      --workers 60 ;
  done
  fairseq-preprocess \
    --only-source \
    ...
    --destdir "../../data-bins/superbizarre/bpe/alpha0/vocab_size16000/spm1/$TASK-bin/label" \
    --workers 60;
done
