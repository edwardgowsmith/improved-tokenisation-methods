...
do
  ...
  do
    ...
    then
      cp "$TASK_DATA_FOLDER/$SPLIT.tsv" "$TASK_DATA_FOLDER/processed_spm1_unigram/$SPLIT.tsv.temp";
    else
      tail -n +2 "$TASK_DATA_FOLDER/$SPLIT.tsv" > "$TASK_DATA_FOLDER/processed_spm1_unigram/$SPLIT.tsv.temp";
    fi

    # Remove unformatted lines from train and dev files for QQP dataset.
    if [[ ( "$TASK" = "QQP") && ( "$SPLIT" != "test" ) ]]
    then
      awk -F '\t' -v NUM_FIELDS=6 'NF==NUM_FIELDS{print}{}' "$TASK_DATA_FOLDER/processed_spm1_unigram/$SPLIT.tsv.temp" > "$TASK_DATA_FOLDER/processed_spm1_unigram/$SPLIT.tsv";
    else
      cp "$TASK_DATA_FOLDER/processed_spm1_unigram/$SPLIT.tsv.temp" "$TASK_DATA_FOLDER/processed_spm1_unigram/$SPLIT.tsv";
    fi
    rm "$TASK_DATA_FOLDER/processed_spm1_unigram/$SPLIT.tsv.temp";
  done
  ...
  do
    ...
    then
      ...
      then
        cut -f"$DEV_LABEL_COLUMN" "$TASK_DATA_FOLDER/processed_spm1_unigram/$SPLIT.tsv"  > "$TASK_DATA_FOLDER/processed_spm1_unigram/$SPLIT.label";
      else
  ...
  rm -rf "../../data-bins/glue/unigram/alpha0/vocab_size16000/spm1/$TASK-bin"

  DEVPREF="$TASK_DATA_FOLDER/processed_spm1_unigram/dev.LANG"
  TESTPREF="$TASK_DATA_FOLDER/processed_spm1_unigram/test.LANG"
  if [ "$TASK" = "MNLI" ]
  then
    DEVPREF="$TASK_DATA_FOLDER/processed_spm1_unigram/dev_matched.LANG,$TASK_DATA_FOLDER/processed_spm1_unigram/dev_mismatched.LANG"
    TESTPREF="$TASK_DATA_FOLDER/processed_spm1_unigram/test_matched.LANG,$TASK_DATA_FOLDER/processed_spm1_unigram/test_mismatched.LANG"
  fi
  ...
  do
    ...
    fairseq-preprocess \
      ...
      --destdir "../../data-bins/glue/unigram/alpha0/vocab_size16000/spm1/$TASK-bin/$LANG" \
      --workers 60 ;
  done
  if [[ "$TASK" !=  "STS-B" ]]
  then
    fairseq-preprocess \
      ...
      --destdir "../../data-bins/glue/unigram/alpha0/vocab_size16000/spm1/$TASK-bin/label" \
      --workers 60;
  else
    # For STS-B output range is converted to be between: [0.0, 1.0]
    mkdir -p "../../data-bins/glue/unigram/alpha0/vocab_size16000/spm1/$TASK-bin/label"
    awk '{print $1 / 5.0 }' "$TASK_DATA_FOLDER/processed_spm1_unigram/train.label" > "../../data-bins/glue/unigram/alpha0/vocab_size16000/spm1/$TASK-bin/label/train.label"
    awk '{print $1 / 5.0 }' "$TASK_DATA_FOLDER/processed_spm1_unigram/dev.label" > "../../data-bins/glue/unigram/alpha0/vocab_size16000/spm1/$TASK-bin/label/valid.label"
  fi
done
