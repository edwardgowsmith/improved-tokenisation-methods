...
class PreTrainedTokenizerFastModified(PreTrainedTokenizerBase):
    """
    ...
    def _convert_encoding(
        ...
        for e in encodings:
            input_ids = [x for x in e.ids if x != 5]
            len_input_ids = len([x for x in input_ids if x != 3])
            encoding_dict["input_ids"].append(input_ids + [3]*(256-len(input_ids)))

            if return_token_type_ids:
                token_type_ids = [0]*len(input_ids)
                z = len(token_type_ids)
                encoding_dict["token_type_ids"].append(token_type_ids + [0]*(256-z))
            if return_attention_mask:
                attention_mask = e.attention_mask[:len_input_ids]
                z = len(attention_mask)
                encoding_dict["attention_mask"].append(attention_mask + [0]*(256-z))
